...
public class SharedTable implements HTableInterface {
    ...
    public void put(final Put put) throws IOException {
        ...
        try {
            ...
            LOG.debug("Checking if cache contains value for row " +  row + " in table "+ tableName);
            ...
            if(rowInCache){
                ...
                if(res.isEmpty()){
                    ...
                    LOG.debug("Storing unique id "+ rowID + " on cache with key " + row + " for table" + tableName);
                    /**
                    ...
                    LOG.debug("An identifier already exists "+ res.getRow() + " with unique key "+ mGet.getUniqueRowId());
                    /**
    ...
    public Result get(Get get) throws IOException {
        ...
        try {
            ...
            if(rowInCache){
                ...
                LOG.debug(Thread.currentThread().getId()+" Retrieved unique id on cache "+ rowID);
                res = handleGet(get, true, convKey(rowID)).getResult();
            }else{
                LOG.debug(Thread.currentThread().getId()+" Key not stored on cache");
    ...
    private MultiGet handleGet(Get get, boolean isCached, byte[] cachedID){
        try {
            ...
            Result res = mGet.getResult();
            LOG.debug(Thread.currentThread().getId()+" Result found is emptpy? " + res.isEmpty());
            if(!res.isEmpty()){
                LOG.debug(Thread.currentThread().getId()+" Storing in cache value "+ mGet.getUniqueRowId());
        ...
        try {
            ...
            if (scan.getStartRow().length != 0 && scan.getStopRow().length != 0) {
                ...
                MultiScan ms = new MultiScan(sharedConfig, connections,
                        requestID, targetPlayer, startRowSecrets, stopRowSecrets);
                ms.startScan();
                return ms;

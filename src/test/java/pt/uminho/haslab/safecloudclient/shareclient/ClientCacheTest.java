...
public class ClientCacheTest {
	 ...
	 * cache. These numbers work as identifier and row. Values can be inserted
	...
	public void test() throws InterruptedException {
		...
		for (String table : tables) {
			...
			Iterator<Cache.Entry<byte[], Long>> it = cacheimpl
					.iterateCache(table);
			boolean found = false;
			while (it.hasNext()) {
				Cache.Entry<byte[], Long> entry = it.next();
				LOG.debug("Searching on table " + table + " the row "
						+ new String(entry.getKey()) + " with val "
						+ entry.getValue());
				...
				for (Thread t : threads) {
					...
					if (cli.getTableName().equals(table)) {
						...
						found |= cli.insertedEntry(entry.getKey(),
								entry.getValue());
					}
				}
			}
	...
	private class Client extends Thread {
		...
		public boolean insertedEntry(byte[] row, Long val) {
			return insertedValues.containsKey(row)
					&& insertedValues.get(row).equals(val);
		}
		...
		public void run() {
			...
			for (int i = 0; i < values.size(); i++) {
				LOG.debug("Going to put on table " + tableName + " the key "
						+ i + " with value " + values.get(i));
				byte[] key = ("" + i).getBytes();
			...
			/**
			 * Each client does a simple test. It simply tests if it has made
			 * every operation by counting the number of ops done in the loop.
			 * The reason for such a simple test is because the unit tests
			 * launches that may write to the same table and override values,
			 * which makes it hard to predict which values will be stored on the
			 * final cache what should be the correct behavior. This test simply
			 * allows to find exception or wrong behavior easier.
			 * */
			if (nOps == values.size()) {
